...
function Primitive({ value }: { value: unknown }) {
  ...
  // bigint serialized as "123n" string already handled above;
...
function CollapsibleNode({
  ...
  return (
    <div>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1 text-left font-mono text-xs hover:text-black"
      >
        <svg
          className={`h-2.5 w-2.5 shrink-0 text-gray-400 transition-transform ${open ? "rotate-90" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={3}
        ...
        {!open && (
          <span className="text-gray-400">{label.endsWith("]") ? "…]" : "…}"}</span>
        )}
      </button>
      {open && <div className="ml-4 border-l border-gray-100 pl-2">{children}</div>}
    </div>
  );
}
    ...
    <CollapsibleNode label="{}" defaultOpen={depth === 0}>
      {entries.map(([key, val]) => (
        <div key={key} className="flex gap-1 font-mono text-xs">
          <span className="shrink-0 font-medium text-gray-700">{key}:</span>
