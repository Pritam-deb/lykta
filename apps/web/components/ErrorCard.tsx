...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={copy}
      title={text}
      className="ml-1 rounded px-1 py-0.5 text-[10px] text-red-400 hover:bg-red-100 hover:text-red-600"
    >
      copy
    </button>
  );
  ...
  return (
    <div className="rounded border border-red-200 bg-red-50 p-4 text-sm">
      ...
      <div className="flex items-center gap-2">
        <span className="rounded-full bg-red-200 px-2.5 py-0.5 text-xs font-semibold text-red-800">
          Error
        </span>
        <span className="font-semibold text-red-900">{title}</span>
      </div>
      ...
      <dl className="mt-3 grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-xs">
        <dt className="font-medium text-red-700">Code</dt>
        <dd className="font-mono text-red-900">{formatCode(error.code)}</dd>

        <dt className="font-medium text-red-700">Program</dt>
